...
1. A supervisor is authored **in-band** (one optional `spec.supervisor` block per
   ...
   each autonomous agent run that subscribes to its live progress and issues
...
2. Add it under `spec:` (a sibling of `steps:`), NOT as a step:
   ...
   spec:
     ...
     supervisor:
       ...
       # criteria: artifact://loop-policy@1
       #                           ↑ or reference a shared steering artifact
...
4. **Policy as an artifact** — instead of inline text, point `criteria` at a
   shared steering artifact (`criteria: artifact://loop-policy@1`) authored in
   `artifacts/`. `steer_message` sets the text injected on a `steer` from a
   **rule** (the LLM path steers with its reason).
