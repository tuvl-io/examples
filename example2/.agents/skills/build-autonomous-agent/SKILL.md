...
1. Add a step of `kind: Agent` with `mode: autonomous`. Unlike `mode: completion`
   ...
   re-decides until done. `agent.tools` is REQUIRED in this mode.
2. Declare the tools — each `ref` names ANOTHER step in the same workflow (an
   ...
   `description:`** (REQUIRED — set it on the step). `parameters` (JSON Schema)
   is optional:
   ```yaml
   - id: triage_agent
     kind: Agent
     ...
     agent:
       ...
       outcome:
         ...
         write: agent_result                       # the single data output
     routes:
       ...
       aborted:         alert_ops                  # supervisor/operator abort
   ```
3. **Every** `outcome.enum` value MUST be mapped in `routes:`, plus the reserved
   abnormal exits `max_iterations` / `budget_exceeded` / `aborted` / `error`.
4. The agent reads context and writes only `outcome.write`. Tool results return
   to the agent; set `writes_context: true` on a tool only if it should also
   mutate the shared workflow context.
5. For data-driven branching after an outcome (e.g. by country), route into a
   ...
   ```
6. **Steering vs skills** — `steering` (always injected) is the agent's
   persistent operating contract; `skills` are capabilities applied when
   relevant. Both take inline text or `artifact://` references to shared
   prose artifacts in `artifacts/`.
