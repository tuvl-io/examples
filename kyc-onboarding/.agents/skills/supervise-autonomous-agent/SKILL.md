...
description: Add a per-workflow supervisor that watches AutonomousAgent runs and can pause/abort/steer them.
options:
...
1. A supervisor is authored **in-band** (one optional `spec.supervisor` block per
   ...
   each `AutonomousAgent` run that subscribes to its live progress and issues
...
2. Add it under `spec:` (a sibling of `steps:`), NOT as a step:
   ...
   spec:
     ...
     supervisor:
       ...
       #                           ↑ scoped .md alternative to inline `criteria`
...
4. **Policy as a file** — instead of inline `criteria`, set `criteria_file` to a
   `.md` path under `agents/<workflow>__supervisor/steering/` (a file wins over
   inline `criteria`; out-of-scope paths are ignored). `steer_message` sets the
   text injected on a `steer` from a **rule** (the LLM path steers with its reason).
...
6. **In Insight** you can skip the YAML: add the off-spine **Supervisor** node from
   ...
   configured models), criteria or a criteria `.md` file, `on_violation`,
